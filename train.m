...
function [name1] = train(name)   % Define a function with name 'train' and an input parameter which is the location of audio sample to save
    ...
    display('MULA BRCAKAP');       % Displays the string "Start Speaking" in command window
    ...
    display('BERHENTI');        % Displays the string "Stop Speaking" in command window
    name1 = getaudiodata(sig);       % Gets the audio data from 'sig' object and stores it in variable named 'name1'
    audiowrite('audio3/train_name.wav',name1,8000);    % Saves the audio sample with '.wav' extension as mentioned in the input parameter
